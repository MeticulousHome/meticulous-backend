...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("rating IN ('good', 'bad', 'unrated')", name='valid_rating_check')
    )
    
    op.create_index('idx_shot_ratings_history_id', 'shot_ratings', ['history_id'])


def downgrade() -> None:
    op.drop_index('idx_shot_ratings_history_id')
