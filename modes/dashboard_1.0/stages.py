...
def get_curve_stages(parameters: dict, start_node: int, end_node: int):
    stages_list = []
    for stage in parameters["stages"]:
        stages_list.append(get_stage(stage, start_node, end_node))
    return stages_list

def get_stage(parameters,start_node,end_node):
        if "kind" in parameters and parameters["kind"] ==  "curve_1.0":
            name = parameters["name"]
            points_controller = parameters["parameters"]["points"]
            stop_weight = parameters["parameters"]["stop_weight"]
            max_time = parameters["parameters"]["stop_time"]
            max_limit_trigger = parameters["parameters"]["max_limit_trigger"]
            interpolation_kind = parameters["parameters"]["interpolation_method"]    

            if "parameters" in parameters and "control_method" in parameters["parameters"]:
                ...
                elif parameters["parameters"]["control_method"] == "pressure":
        ...
        elif "kind" in parameters and parameters["kind"] == "spring_1.0":
            spring_template = {"spring":1}
            return spring_template
        
        else:
            print("No valid type: {}".format(parameters.get("kind")))
            return None
...
if __name__ == '__main__':
    ...
    stage_modified = get_curve_stages(payload, 300, 301)
