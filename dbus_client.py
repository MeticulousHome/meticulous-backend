...
class AsyncDBUSClient(object):
    def __init__(self):
        ...
        self.new_signal_subscription('de.pengutronix.rauc.Installer',
                                     'Completed',
                                     self.rauc_update_complete)
        
        self.new_property_subscription('de.pengutronix.rauc.Installer','Progress',self.update_progress)
        self.new_property_subscription('de.pengutronix.rauc.Installer','LastError',self.report_error)
    ...
    async def update_progress(self,  connection, sender_name, object_path, property_interface, attribute, status):
        notification_message = f"Updating OS:\nStatus: {status}"
        NotificationManager.add_notification(
            Notification(message=notification_message, responses=[], image=notification_image)
        )
        self.logger.info(f"property: [{attribute}], is [{status}]")

    async def report_error(self,  connection, sender_name, object_path, property_interface, attribute, status):
        if status == "":
            return
        error_rauc_updating = True
    ...
    async def rauc_update_complete(self,  connection, sender_name, object_path, property_interface, attribute, status):
        global error_rauc_updating
        if error_rauc_updating == True:
            error_rauc_updating = False
            return 
