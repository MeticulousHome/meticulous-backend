...
# ? for debug purposes
def add_testing_str(event: Event, hint: Hint) -> Event | None:
    if hint.get("exc_info", [None])[0] == TypeError:
        event["extra"][
            "Dev message"
        ] = "Testing sentry disable and re-enable functions (IGNORE)"
        event["extra"]["sentry_status"] = "Prior to disabling"
    if hint.get("exc_info", [None])[0] == ValueError:
        event["extra"][
            "Dev message"
        ] = "Testing sentry disable and re-enable functions (IGNORE)"
        event["extra"]["sentry_status"] = "sentry re-enabled"
    return event
