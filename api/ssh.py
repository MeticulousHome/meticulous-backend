...
from config import MeticulousConfig, CONFIG_SYSTEM, SSH_ENABLED
...
class SSHHandler(BaseHandler):
    def get(self):
        ...
        self.write(
            json.dumps({"ssh_enabled": MeticulousConfig[CONFIG_SYSTEM][SSH_ENABLED]})
        )
    ...
    def post(self):
        ...
        try:
            ...
            if "ssh_enabled" not in settings:
                self.set_status(400)
                self.write(
                    {"status": "error", "message": "Missing ssh_enabled parameter"}
                )
                return
            ...
            if SSHManager.set_ssh_state(enabled):
                ...
                self.write({"status": "success", "ssh_enabled": enabled})
            else:
                self.set_status(500)
                self.write(
                    {"status": "error", "message": "Failed to update SSH service state"}
                )

        except json.JSONDecodeError:
            self.set_status(400)
            self.write({"status": "error", "message": "Invalid JSON in request body"})
